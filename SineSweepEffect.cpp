...
SineSweepEffect::SineSweepEffect() :
    m_refCount(1)
{
...
HRESULT __stdcall SineSweepEffect::CreateSineSweepImpl(_Outptr_ IUnknown** ppEffectImpl)
{
    // Since the object's refcount is initialized to 1, we don't need to AddRef here.
    *ppEffectImpl = static_cast<ID2D1EffectImpl*>(new (std::nothrow) SineSweepEffect());
...
HRESULT SineSweepEffect::Register(_In_ ID2D1Factory1* pFactory)
{
...
IFACEMETHODIMP SineSweepEffect::Initialize(
    ...
    )
{
    ...
    if (SUCCEEDED(hr))
    {
        hr = pEffectContext->LoadPixelShader(GUID_SineSweepPixelShader, data, size);
    }
...
HRESULT SineSweepEffect::UpdateConstants()
{
...
IFACEMETHODIMP SineSweepEffect::PrepareForRender(D2D1_CHANGE_TYPE changeType)
{
...
IFACEMETHODIMP SineSweepEffect::SetGraph(_In_ ID2D1TransformGraph* pGraph)
{
    return E_NOTIMPL;
}
...
IFACEMETHODIMP SineSweepEffect::SetDrawInfo(_In_ ID2D1DrawInfo* pDrawInfo)
{
    m_drawInfo = pDrawInfo;

    return m_drawInfo->SetPixelShader(GUID_SineSweepPixelShader);
}

// Calculates the mapping between the output and input rects.
IFACEMETHODIMP SineSweepEffect::MapOutputRectToInputRects(
...
IFACEMETHODIMP SineSweepEffect::MapInputRectsToOutputRect(
...
IFACEMETHODIMP SineSweepEffect::MapInvalidRect(
    UINT32 inputIndex,
...
IFACEMETHODIMP_(UINT32) SineSweepEffect::GetInputCount() const
{
    return 0;
}
...
IFACEMETHODIMP_(ULONG) SineSweepEffect::AddRef()
{
    m_refCount++;
    return m_refCount;
}

IFACEMETHODIMP_(ULONG) SineSweepEffect::Release()
{
    m_refCount--;
...
IFACEMETHODIMP SineSweepEffect::QueryInterface(
    _In_ REFIID riid,
...
HRESULT SineSweepEffect::SetCenter(D2D1_POINT_2F center)
{
...
D2D1_POINT_2F SineSweepEffect::GetCenter() const
{
    return m_constants.center;
}

HRESULT SineSweepEffect::SetWavelengthHalvingDistance(float dist)
{
...
float SineSweepEffect::GetWavelengthHalvingDistance() const
{
    return m_constants.wavelengthHalvingDistance;
}

HRESULT SineSweepEffect::SetInitialWavelength(float wavelength)
{
...
float SineSweepEffect::GetInitialWavelength() const
{
    return m_constants.initialWavelength;
}

HRESULT SineSweepEffect::SetWhiteLevelMultiplier(float multiplier)
{
...
float SineSweepEffect::GetWhiteLevelMultiplier() const
{
